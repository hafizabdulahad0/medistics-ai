...
const Pricing = () => {
  ...
  const [currency, setCurrency] = useState('PKR'); // New state for currency: 'PKR' or 'USD'

  // Define pricing plans data with monthly and yearly options for both currencies
  const plans = [
    {
      name: 'Free',
      monthly: {
        PKR: { price: '0', features: ['Static MCQs', 'Basic Dashboard', 'Limited Access'] },
        USD: { price: '0', features: ['Static MCQs', 'Basic Dashboard', 'Limited Access'] }
      },
      yearly: {
        PKR: { price: '0', features: ['Static MCQs', 'Basic Dashboard', 'Limited Access'] },
        USD: { price: '0', features: ['Static MCQs', 'Basic Dashboard', 'Limited Access'] }
    ...
    },
    {
      name: 'Basic',
      monthly: {
        PKR: { price: '250', features: ['AI Test Generator', 'Practice MCQs', 'Battle Arena', 'Analytics'] },
        USD: { price: '0.99', features: ['AI Test Generator', 'Practice MCQs', 'Battle Arena', 'Analytics'] }
      },
      yearly: {
        PKR: { price: '2400', features: ['AI Test Generator', 'Practice MCQs', 'Battle Arena', 'Analytics', 'Save PKR 600'] },
        USD: { price: '10.99', features: ['AI Test Generator', 'Practice MCQs', 'Battle Arena', 'Analytics', 'Save $0.89'] } // Updated USD saving
      },
    ...
    },
    {
      name: 'Premium',
      monthly: {
        PKR: { price: '500', features: ['AI Chatbot', 'Voice Input', 'Unlimited Battles', 'Priority Support'] },
        USD: { price: '1.99', features: ['AI Chatbot', 'Voice Input', 'Unlimited Battles', 'Priority Support'] }
      },
      yearly: {
        PKR: { price: '5200', features: ['AI Chatbot', 'Voice Input', 'Unlimited Battles', 'Priority Support', 'Save PKR 800'] },
        USD: { price: '19.99', features: ['AI Chatbot', 'Voice Input', 'Unlimited Battles', 'Priority Support', 'Save $3.89'] }
      },
  ...
  // Placeholder for display name, consistent with dashboard
            ...
            <Card key={index} className={`relative ${plan.gradient} ${plan.border} ${plan.popular ? 'border-purple-500 dark:border-purple-400 scale-105 shadow-2xl' : 'shadow-lg'} transition-all duration-500 hover:shadow-2xl hover:scale-105 animate-scale-in group overflow-hidden`} style={{ animationDelay: `${index * 0.2}s` }}>
              ...
              <CardHeader className="text-center relative z-10">
                ...
                <div className="mt-4">
                  <span className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">
                    {currency === 'PKR' ? 'PKR ' : '$'}
                    {isMonthly ? plan.monthly[currency].price : plan.yearly[currency].price}
                  </span>
              ...
              <CardContent className="relative z-10">
                <ul className="space-y-3 mb-6">
                  {(isMonthly ? plan.monthly[currency].features : plan.yearly[currency].features).map((feature, idx) => (
                ...
                {/* Link to Checkout page with query parameters */}
                <Link
                  to={`/checkout?planName=${plan.name}&price=${isMonthly ? plan.monthly[currency].price : plan.yearly[currency].price}&duration=${isMonthly ? 'monthly' : 'yearly'}&currency=${currency}`}
                >
